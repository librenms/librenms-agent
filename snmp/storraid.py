...
blocks waiting for storcli.  Data is at most ~60 s old, well within
the LibreNMS default 300 s polling interval.
----------------------------------------------------------------------
...
def parse_controllers(raw):
    ...
    for entry in raw.get("Controllers", []):
        ...
        if not ctrl_ok(entry):
            controllers.append({
                "id": cid, "model": "Unknown", "serial": "N/A",
                "firmware": "N/A", "memory_mb": "N/A", "state": "Failed",
                "severity": 2, "temperature": None,
                "vd_count": 0, "pd_count": 0, "bbu": None, "is_hba": True,
            })
            continue
        ...
        model    = (basics.get("Model")
                    or resp.get("Product Name", "Unknown")).strip()
        serial   = (basics.get("Serial Number")
                    or resp.get("Serial Number", "N/A")).strip()
        firmware = (version.get("Firmware Version")
                    or resp.get("FW Version")
                    or version.get("Firmware Package Build")
                    or resp.get("FW Package Build", "N/A")).strip()
        ...
        if is_hba:
            ...
            ctrl_state_raw = (status.get("Controller Status")
                              or resp.get("Controller Status", ""))
            if ctrl_state_raw:
        ...
        mem_val = _deep_find(resp,
            ("On Board Memory Size", "Memory Size", "Memory"), bad=("N/A", ""))
        mem = str(mem_val) if mem_val is not None else ("0MB" if is_hba else "N/A")

        temp_val = _deep_find(resp,
            ("ROC temperature(Degree Celsius)", "ROC temperature(Degree Celcius)",
             "Ctrl Temp", "ROC Temp"), bad=("N/A", ""))
        try:
            temp_c = int(temp_val) if temp_val is not None else None  # type: Optional[int]
        except (TypeError, ValueError):
            temp_c = None
    ...
    for sub, bbu_type in (("/call/bbu", "BBU"), ("/call/cv", "CacheVault")):
        ...
        for entry in raw.get("Controllers", []):
            ...
            if cid < 0 or cid in bbu_by_ctrl:
                continue
            resp = get_resp(entry)
            info_list = (resp.get("BBU_Info") or resp.get("BBU Info")
                         or resp.get("Cachevault_Info") or resp.get("CacheVault_Info")
                         or resp.get("Cachevault Info") or [])
            ...
            bbu_by_ctrl[cid] = {
                ...
                "charge_pct":  str(b.get("Charge",
                               b.get("Relative State of Charge",
...
def parse_virtual_disks(storcli, raid_ctrl_ids, raw):
    ...
    for entry in raw.get("Controllers", []):
        ...
        for row in get_resp(entry).get("Virtual Drives", []):
            ...
            progress  = _get_vd_progress(storcli, cid, vd_idx) if state_sev == 1 else None
            vds.append({
                "controller":   cid,
                "id":           dg_vd,
    ...
    for entry in raw.get("Controllers", []):
        ...
        if not isinstance(pd_rows, list) or not pd_rows:
            ...
            for k, v in resp.items():
                if (k.startswith("Drive /c") and "Detailed" not in k
        ...
        for row in pd_rows:
            ...
            temp_raw = (drv_state.get("Drive Temperature")
                        or attrs.get("Drive Temperature")
                        or attrs.get("Temperature", ""))
            try:
                temp_c = int(str(temp_raw).strip().split("C")[0].strip())  # type: Optional[int]
            ...
            pds.append({
                ...
                "serial":       (attrs.get("Serial Number")
                                 or attrs.get("SN")
                                 or row.get("SN", "N/A")).strip(),
                "firmware":     (attrs.get("Firmware Revision")
...
def _extract_pd_detail(resp, detail_by_key, cid, eid, slot):
    ...
    detail    = (detail_by_key.get("{} - Detailed Information".format(base))
...
def main():
    output = {
        ...
        "summary": {
            "overall_severity": 0,
            "ctrl_count": 0, "vd_count":  0, "pd_count":  0,
            "ctrl_ok":    0, "ctrl_warn": 0, "ctrl_crit": 0,
            "vd_ok":      0, "vd_warn":   0, "vd_crit":   0,
            "pd_ok":      0, "pd_warn":   0, "pd_crit":   0,
        },
    }

    storcli = find_storcli()
    if not storcli:
        err = ("storcli not found. Install storcli64 to one of: "
               + ", ".join(STORCLI_PATHS))
        output["error"] = err
    ...
    raid_ctrl_ids = [c["id"] for c in controllers
                     if "error" not in c and not c.get("is_hba")]
    all_ctrl_ids  = [c["id"] for c in controllers if "error" not in c]

    if not all_ctrl_ids:
        err = (controllers[0].get("error", "No controllers found")
               if controllers else "No controllers found")
        output["error"] = err
        cleanup_storcli_log()
        write_output(_envelope(1, err, output))
        sys.exit(0)
    ...
    bbu_needed = any(
        ...
        for e in raw_ctrl.get("Controllers", []) if ctrl_ok(e)
    )
    if bbu_needed:
    ...
    for c in controllers:
        ...
        all_sev.append(sev)
        if sev == 0:   s["ctrl_ok"]   += 1
        elif sev == 1: s["ctrl_warn"] += 1
        else:          s["ctrl_crit"] += 1
        if c.get("bbu"):
